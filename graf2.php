...
if ($dados['DESCR_REQUERIMENTO'] !== "" && $dados['DESCR_REQUERIMENTO'] !== null) {
    $dados['DESCR_REQUERIMENTO'];
        } else {
            $dados['DESCR_REQUERIMENTO'] = "NÃO HOUVE REQUERIMENTO NESSE PERÍODO";
        }

// Conta a ocorrência de cada valor no array $NM_USU e armazena em $contagemPorUsuario
$contagemPorUsuario = array_count_values($NM_USU);

// Obtém as chaves (labels) e valores (data) do array $contagemPorUsuario
$labels = array_keys($contagemPorUsuario);
$data = array_values($contagemPorUsuario);

// Prepara as strings para uso no JavaScript, escapando as aspas
$labelStrings = [];
foreach ($labels as $label) {
    $labelStrings[] = "'" . addslashes($label) . "'";
}
$labelsString = implode(', ', $labelStrings);
...
<div class="col-4">
<input class="caixa-texto" type="text" disabled value="<?php echo "Requerimento Por Atendente"?>">
</div>
